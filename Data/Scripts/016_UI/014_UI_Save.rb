def pbEmergencySave
  ...
  if SaveData.exists?(SaveData::SAVE_INDEX)
    File.open(SaveData::FILE_PATHS[SaveData::SAVE_INDEX], "rb") do |r|
      File.open(SaveData::FILE_PATHS[SaveData::SAVE_INDEX] + ".bak", "wb") do |w|
        loop do
          s = r.read(4096)
          break if !s
          w.write(s)
...
class PokemonSaveScreen
  ...
  def pbSaveScreen
    ...
    if pbConfirmMessage(_INTL("Would you like to save the game?"))
      if SaveData.exists?(SaveData::SAVE_INDEX) && $game_temp.begun_new_game
